...
SRC = parser/src/
BIN = parser/bin/
PARSER_SRC_FILES = $(wildcard parser/src/VC*.c)
PARSER_OBJ_FILES = $(patsubst parser/src/VC%.c, parser/bin/VC%.o,$(PARSER_SRC_FILES))

parser: libvcparser.so

libvcparser.so: $(PARSER_OBJ_FILES) $(BIN)LinkedListAPI.o
...
clean:
	rm -rf $(BIN)StructListDemo $(BIN)xmlExample $(BIN)*.o *.so
